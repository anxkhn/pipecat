...
class FastAPIWebsocketInputTransport(BaseInputTransport):
    ...
    async def cleanup(self):
        ...
        section in CONTRIBUTING.md): it runs even when no EndFrame/CancelFrame
        reaches the transport, so it stops the receive loop and disconnects the
        client here too.
        """
        await super().cleanup()
        await self._teardown()
...
class FastAPIWebsocketOutputTransport(BaseOutputTransport):
    ...
    async def cleanup(self):
        ...
        This is the guaranteed teardown hook (see the Processor Lifecycle
        section in CONTRIBUTING.md): it runs even when no EndFrame/CancelFrame
        reaches the transport, so it disconnects the client here too. The
        client's ``disconnect()`` is idempotent, so repeating it after
        ``stop()``/``cancel()`` is safe.
        """
        await super().cleanup()
        await self._client.disconnect()
