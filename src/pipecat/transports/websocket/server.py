...
class SingleClientWebsocketServerInputTransport(BaseInputTransport):
    ...
    async def cleanup(self):
        ...
        Cancels the server and monitor tasks. The frame-driven ``stop``/``cancel``
        paths do this promptly; this repeats it idempotently so it happens even if
        those frames never arrive. See the Processor Lifecycle section in
        ``CONTRIBUTING.md``.
        """
        await super().cleanup()
        await self._stop_tasks()
