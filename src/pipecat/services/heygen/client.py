...
class HeyGenClient:
    ...
    async def cleanup(self) -> None:
        ...
        This is the guaranteed teardown path for the client (invoked from the
        owning processor's ``cleanup()``). It cancels every task and closes every
        connection, and is idempotent so it can also be called from
        :meth:`stop`. See the Processor Lifecycle section in CONTRIBUTING.md.
        """
        try:
    ...
    async def stop(self) -> None:
        ...
        Delegates to the idempotent :meth:`cleanup`, which is the guaranteed
        teardown path.
        """
